...
const MENU_ITEMS = [
  ...
  { label: 'Insumos', icon: <InventoryIcon /> },
  { label: 'Tareas', icon: <PlaylistAddCheckIcon /> },
  { label: 'Visitas', icon: <GroupsIcon /> },
];
...
export const CoordinacionPage = () => {
  ...
  const [editUserTarget, setEditUserTarget] = useState(null); // { id, nombre, idVoluntaria }
  ...
  useEffect(() => {
    if (volunteer?.postAssistanceDelete != null) {
      dispatch(showLoading(false));
      toastOk('Asistencia dada de baja.');
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0, width: '100%', position: 'relative' }}>
        ...
        <Box sx={{ mx: 1, mb: 2 }}>
          ...
          <Paper elevation={0} sx={{ p: 2, borderRadius: 2, border: '1px solid rgba(0,0,0,0.08)' }}>
            ...
            {tab === 2 && (
              <Box>
                {/* Lista de usuarios */}
                <Typography variant="subtitle2" sx={{ mb: 1, color: VIOLET, fontWeight: 700 }}>
                ...
                {Array.isArray(usuariosList) && usuariosList.length > 0 && (
                  <Box sx={{ mb: 2 }}>
                    {usuariosList.map((u) => {
                      ...
                      const nombre = `${u.nombreVoluntaria ?? u.nombre ?? '—'}`.trim();
                      const apellido = `${u.apellidoVoluntaria ?? u.apellido ?? ''}`.trim();
                      return (
                        <Box
                          key={uid}
                          sx={{
                        ...
                        >
                          ...
                          <Box sx={{ display: 'flex', gap: 0.5, flexShrink: 0 }}>
                            ...
                            <IconButton
                              ...
                              onClick={() =>
                                openConfirm(
                                  `¿Eliminar acceso de "${nombre} ${apellido}"?`,
                                  () => {
                                    dispatch(showLoading(true));
                                    dispatch(postUsuarioDelete(uid));
                                  },
                                )
                              }
                            >
                ...
                {Array.isArray(volsSinUsuario) && volsSinUsuario.length > 0 ? (
                  ...
                  <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
                    Todas las voluntarias ya tienen acceso al sistema.
                  </Typography>
                ) : (
                  <TextField
                ...
                {(!Array.isArray(volsSinUsuario) || volsSinUsuario.length > 0) && (
                  <>
                    ...
                    <Button
                      variant="contained"
                      disabled={!usuarioForm.idVoluntaria || !usuarioForm.dni || !usuarioForm.contrasena}
                      onClick={() => {
                        dispatch(showLoading(true));
                        dispatch(
                          postUsuario({
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle sx={{ fontWeight: 600, color: '#152C70' }}>
          Editar acceso
        </DialogTitle>
