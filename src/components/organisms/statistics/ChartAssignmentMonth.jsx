...
const SLICE_COLORS = [
    ...
    'rgba(122, 101, 155, 0.82)',
    'rgba(59, 130, 246, 0.82)',
];

export function ChartAssignmentMonth({ title, statisticsAssignment }) {
    const labels = statisticsAssignment?.map((d) => NOMBRES_MES[(d.mes ?? 1) - 1]) ?? [];
    const values = statisticsAssignment?.map((d) => d.cantidadAsignaciones) ?? [];
    ...
    const options = {
        ...
        plugins: {
            ...
            title: { display: false },
            tooltip: { callbacks: { label: (ctx) => ` ${ctx.label}: ${ctx.parsed} asignaciones` } },
        },
    };
